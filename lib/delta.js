...
exports.create = function(options, cb) {
  ...
  if (!_o.name) {
    return _cb(new Error('Missing distribution name'));
  }

  if (!_o.time) {
    return _cb(new Error('Missing mean lifetime'));
  }

  _o.date = _o.date || new Date().getTime();
  var delta = new Delta(_o.name);
  delta.init(_o, _cb);
};
...
exports.get = function(name, cb) {
  var _name = name;
  var _cb = cb;
  var _delta = new Delta(_name);

  if (typeof _cb !== 'function') {
    throw new Error('Invalid callback provided!');
  }

  _delta.exists(function(e, exists) {
    if (!exists || e) {
      return _cb(new Error('Delta does not exists'))
    }
