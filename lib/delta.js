...
Delta.prototype.fetch = function(o, cb) {
  ...
  this.doFetch(fetchOpts, function(e, res) {
    ...
    if (!o.bin) {
      ...
      for (var i in count) {
        norm_v = norm[i];
        value = (typeof norm_v === 'undefined') ? 0 : parseFloat(count[i]).toFixed(10) / parseFloat(norm_v).toFixed(10);
      ...
      if (!primary) {
        ...
        var score = parseFloat(secondary).toFixed(10) / parseFloat(primary).toFixed(10);
