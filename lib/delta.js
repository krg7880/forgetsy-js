...
/**
Creates a new Primary and Secondary
Set to trend.
@param {Object} o Configuration object
@param {Function} cb Function to invoke when execution
completes.
*/
...
/**
Fetches the primary and secondary set; returning
their scores.
@param {Object} o
@param {Function} cb 
*/
...
Delta.prototype.fetch = function(o, cb) {
  ...
  this.doFetch(fetchOpts, function(e, res) {
    ...
    if (!o.bin) {
      ...
      for (var i in count) {
        norm_v = norm[i];
        value = (typeof norm_v === 'undefined') ? 0 : parseFloat(count[i]) / parseFloat(norm_v);
        console.log(value, count[i], norm_v, 'div', count[i] / norm_v);
        results[i] = value; 
      }
