...
const LAST_DECAYED_KEY = '_last_decay_key';
...
Set.prototype.fetch = function(options) {
	...
	if (options.decay && options.scrub) {
		this.doScrubAndDecay(this.doFetch);
	} else if (options.decay) {
		this.doDecay(this.doFetch);
	} else if (options.scrub) {
		this.doScrub(this.doFetch);
	}
};

Set.prototype.doFetch = function() {
	if (this.options.bin) {
		client.zscore([this.key, this.options.bin], function(e, res) {
			if (e) {
				self.emit(ERROR, e);
			} else {
				self.emit(FETCH_COMPLETE, res);
			}
		});
	} else {
		this.fetchRaw();
